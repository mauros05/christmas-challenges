...
def identify_numbers(words)
  words_array = split_variable(words)
  new_words_arary = []
  words_array.each do |word|
    new_words_arary <<  separe_words(word)
  end
  p new_words_arary

  sumatory(new_words_arary)
end

def separe_words(word)
  match = word.scan(/(?=(\d|zero|one|two|three|four|five|six|seven|eight|nine))/).flatten
  new_array = []
  match.each do |word|
    if NUMBERS.has_key?(word)
      new_array << NUMBERS[word]
    else
      new_array << word.to_i
    end
  end
  correct_caracters(new_array)
end
