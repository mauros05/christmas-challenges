variable = "Game 1: 4 red, 8 green; 8 green, 6 red; 13 red, 8 green; 2 blue, 4 red, 4 green
...
def obtain_the_high_value_of_each_position(array)
  matching_games = []
...
def obtain_bigger_numbers(input)
  max_values_per_game = []

  input.each do |game_array|
    max_values = [0, 0, 0] # Inicializa con ceros para cada una de las tres posiciones

    game_array[1..-1].each do |number_array|
      number_array.each_with_index do |number, index|
        max_values[index] = number if number > max_values[index]
      end
    end

    max_values_per_game << [game_array[0], max_values]
  end

  max_values_per_game
end

def multipli_values(array)
  multiplied_values = []
  array.each do |values|
    values.each do |other_values|
      if other_values.kind_of?(Array)
        multiplied_values << other_values.inject(:*)
      end
    end
  end
  multiplied_values
...
def sum_multiplied_values(array)
  array.inject(0,:+)
end
...
def final_game(input)
  ...
  bigger_numbers = obtain_bigger_numbers(orginize_array)
  mult_values = multipli_values(bigger_numbers)
  sum_multiplied_values(mult_values)
end
